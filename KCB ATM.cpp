...
GROUP MINIPROJECT
TITLE: ATM EMULATOR
IDEA GENERATED,CREATED,REVIEWED AND DESIGNED BY:
TEAM LEADER: BONFACE OKELLO
CORDINATOR: MARGARET ATEMO
GENERAL SECRETARY: FATHI ALI

DEDICATED TO;
GROUP: FRIENDS TO ELDORADO
OTHER GROUP MEMBERS:(RONALD MBAYI,GRIVIN OCHIENG,WINNIE RUGURU,MERCY KOECH,JOY MALIA,ABIGAEL TONUI,DENNIS MUSEMBI,MACKLINE NAINI,REWAL OBOTE)
OVERAL LEADER: ENG. BIBIYE JUMA
ASSISTED BY: ENG. AGGREY SHITSHUKANE

ABOUT GROUP(FRIENDS TO ELDORADO)
GROUP FORMED TO INVESTIGATE:
CONTROL: PLCs
EMBEDDED SYSTEMS: BASED ON ATMEGA MICROCONTROLLERS USING ARDUIDO UNO DEV BOARD
DESIGN: AUTODESK FUSION 360 AND AUTOCAD
...
// Global functions
//This function check to ensure you use a four number pin by counting the number of digits in a pin
//It is a function of return type int
// ret is a place holder for argument pin
...
void createAccount(){
	...
	cin>>Name;
	if(Name.length()>=3&&Name.length()<=32){//This is input validation to ensure user entered name is atleast a 3 letter word
		printf("Please enter your Pin: \n");
		cin>>userPin;
		if(pinCheck(userPin)==4){// Pin validation 
...
void logIn(){
	...
	printf("Please enter your Pin: ");
	cin>>Pin;
	if(Pin==userPin){// Compares pin entered and pin stored when creating account
		mainMenu();
	}
	else{
...
//Main function
int main(){
	do{//This part iterates the code until a given condition defined by while loop is met
	intro();
	if(option=='a'){
		createAccount();
